...
PermissionDef testPermDef = {
    ...
    .grantMode = 1,
    .availableLevel =
        OHOS::Security::AccessToken::ATokenAplEnum::APL_SYSTEM_BASIC,
    .label = "label",
    .labelId = 1,
...
template <class T> T GetData() {
  ...
  if (g_baseFuzzData == nullptr ||
      objectSize > g_baseFuzzSize - g_baseFuzzPos) {
    return object;
  }
  errno_t ret =
      memcpy_s(&object, objectSize, g_baseFuzzData + g_baseFuzzPos, objectSize);
  if (ret != EOK) {
    return {};
  }
...
public:
  AccessToken() {
    ...
    AccessTokenIDEx tokenIdEx =
        AccessTokenKit::AllocHapToken(testInfoParms, testPolicyPrams);
...
public:
  AccessTokenInternetInfo() {
    currentID_ = GetSelfTokenID();
    AccessTokenIDEx tokenIdEx =
        AccessTokenKit::AllocHapToken(testInfoParms, testInternetPolicyPrams);
    accessID_ = tokenIdEx.tokenIdExStruct.tokenID;
    SetSelfTokenID(accessID_);
  }
  ~AccessTokenInternetInfo() {
...
int32_t OnRemoteRequest(uint32_t code, MessageParcel &data) {
  ...
  return DelayedSingleton<EthernetService>::GetInstance()->OnRemoteRequest(
